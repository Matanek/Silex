...
test "LLVM emitter ignores unsupported helpers outside the main call graph" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var frontend = Silex.Frontend.init(allocator);
    const compilation = try frontend.compile(
        \\func unused() { print("outside executable surface") }
        \\func main() { print(true) }
    );
    const text = try emit(allocator, compilation.ir);
    try std.testing.expect(std.mem.containsAtLeast(u8, text, 1, "@.fmt.boolean"));
    try std.testing.expect(!std.mem.containsAtLeast(u8, text, 1, "outside executable surface"));
}

test "LLVM emitter admits only proven scalar math boundaries" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var frontend = Silex.Frontend.init(allocator);
    const compilation = try frontend.compile(
        \\func external(value:float) float { return value }
        \\func main() { print(external(4.0) > 0.0) }
    );
    ...
    var boundary: Silex.Boundary.Function = .{
        .name = "sqrt",
    ...
    const text = try emitWithBoundaries(allocator, program, &.{boundary});
    try std.testing.expect(std.mem.containsAtLeast(u8, text, 1, "declare float @sqrtf(float)"));
    try std.testing.expect(std.mem.containsAtLeast(u8, text, 1, "call float @sqrtf(float"));

    boundary.provider = "Boundary.Custom";
    boundary.package_private = true;
    try std.testing.expectError(error.UnsupportedInstruction, emitWithBoundaries(allocator, program, &.{boundary}));
}
