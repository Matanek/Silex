...
pub fn classify(tag: std.meta.Tag(Silex.Ir.Instruction)) Support {
    return switch (tag) {
        ...
        .constant_bytes, .class_cast, .class_test, .storage_init, .protocol_init, .protocol_test, .protocol_extract, .enum_raw, .list_edit, .collection_slice, .string_address, .string_byte_count, .string_byte_at, .string_from_bytes, .address_load, .address_store, .reference_optional, .format_value, .string_concat, .indirect_call, .boundary_indirect_call, .dynamic_call, .mutex_lock, .mutex_unlock => .unsupported,
    };
}
