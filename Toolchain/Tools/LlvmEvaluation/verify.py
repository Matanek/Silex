...
def main():
    ...
    cases = {
        ...
        "LlvmEvaluation/OwningCollectionReference.sx": "11\n24\n1\n4\n",
    ...
    for relative in ["LlvmEvaluation/Rounding.sx", "ReferenceAliasing.sx", "OwningCollectionCopy.sx", "LlvmEvaluation/Lifetime.sx", "LlvmEvaluation/OptionalValues.sx", "LlvmEvaluation/ClassOwnership.sx", "LlvmEvaluation/ClassFieldStore.sx", "LlvmEvaluation/IndirectCalls.sx", "LlvmEvaluation/Mutex.sx", "LlvmEvaluation/EmbeddedBytes.sx", "LlvmEvaluation/AggregateOptional.sx", "LlvmEvaluation/RichClassStorage.sx", "LlvmEvaluation/ClassFinalizers.sx", "LlvmEvaluation/OwnedStringList.sx", "LlvmEvaluation/PlainStructureEquality.sx", "LlvmEvaluation/RefuseCallback.sx", "LlvmEvaluation/OwningCollectionReference.sx", "LlvmEvaluation/FloatToInteger.sx", "LlvmEvaluation/MixedIntegerShifts.sx", "LlvmEvaluation/RichListAppendClear.sx", "LlvmEvaluation/PlainEnum.sx", "LlvmEvaluation/PayloadEnum.sx", "LlvmEvaluation/StringLiterals.sx", "LlvmEvaluation/StringBytes.sx", "LlvmEvaluation/ListAppendClear.sx", "LlvmEvaluation/StringConcat.sx", "LlvmEvaluation/FormatValues.sx", "LlvmEvaluation/CollectionSlice.sx", "LlvmEvaluation/StringFromBytes.sx", "LlvmEvaluation/RawEnum.sx", "LlvmEvaluation/ProtocolValues.sx", "LlvmEvaluation/StorageInitialization.sx"]:
    ...
    owning_reference_metadata = json.loads(Path(str(output/"OwningCollectionReference-O0")+".json").read_text())
    owning_reference_llvm = (Path(owning_reference_metadata["artifact_directory"])/"raw.ll").read_text()
    for fragment in [
        ".owners = add i64",
        ".shared = icmp ne i64",
        "label %collection.detach",
        ".detached.data = insertvalue",
        "call fastcc void @sx_drop",
        ".data = phi ptr",
    ]:
    ...
    rich_list_metadata = json.loads(Path(str(output/"RichListAppendClear-O0")+".json").read_text())
    rich_list_llvm = (Path(rich_list_metadata["artifact_directory"])/"raw.ll").read_text()
    for fragment in [
        "call fastcc void @sx_string_retain",
        "call void @llvm.memcpy.p0.p0.i64",
        ".appended = getelementptr %sx.type.",
        "store %sx.type.",
        "call fastcc void @sx_string_drop",
        "call fastcc void @sx_drop",
    ]:
