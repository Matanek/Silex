...
pub const Lexer = struct {
    ...
    fn currentPosition(self: *const Lexer) Source.Position {
        return .{ .offset = self.index, .line = self.line, .column = self.column, .file = self.file };
    }
...
test "reserve visibility keywords" {
    var lexer = Lexer.init("private internal local protected public pub sub");
...
test "reserve super keyword" {
    ...
    try std.testing.expectEqual(TokenTag.keyword_super, (try lexer.next()).tag);
}
...
test "recognize reserved copy keyword" {
    var lexer = Lexer.init("copy value");
    try std.testing.expectEqual(TokenTag.keyword_copy, (try lexer.next()).tag);
    try std.testing.expectEqual(TokenTag.identifier, (try lexer.next()).tag);
}
...
test "recognize read reference prefix and released borrow identifier" {
    var lexer = Lexer.init("@ borrow");
    try std.testing.expectEqual(TokenTag.at, (try lexer.next()).tag);
    try std.testing.expectEqual(TokenTag.identifier, (try lexer.next()).tag);
}
...
test "preserve source offset bounds and file identity" {
    var lexer = Lexer.initFile("// note\nvalue", 7);
    const token = try lexer.next();
    try std.testing.expectEqual(@as(usize, 8), token.position.offset);
    try std.testing.expectEqual(@as(usize, 2), token.position.line);
    try std.testing.expectEqual(@as(usize, 1), token.position.column);
    try std.testing.expectEqual(@as(usize, 7), token.position.file);
    try std.testing.expectEqual(@as(usize, 8), token.start);
    try std.testing.expectEqual(@as(usize, 13), token.end);
}

test "keep native as an ordinary identifier" {
    var lexer = Lexer.init("native func");
    const token = try lexer.next();
    try std.testing.expectEqual(TokenTag.identifier, token.tag);
    try std.testing.expectEqualStrings("native", token.lexeme);
    try std.testing.expectEqual(TokenTag.keyword_func, (try lexer.next()).tag);
}
