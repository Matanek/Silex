...
const canonical = ValueOperands.canonical;
const rewriteInstruction = ValueOperands.rewriteInstruction;
const countUses = ValueOperands.countUses;
const countTerminatorUses = ValueOperands.countTerminatorUses;
const instructionResult = ValueOperands.instructionResult;
const BranchSnapshots = @import("BranchSnapshots.zig");
...
pub fn optimizeWithOptions(allocator: Allocator, program: Ir.Program, options: Options) !Ir.Program {
    ...
    if (options.disabled != .branch_snapshot_sinking) {
        current = try BranchSnapshots.optimize(allocator, current);
        try verifyAfterPass(allocator, current, options);
    }
    if (options.stop_after == .branch_snapshot_sinking) return current;
...
test {
    ...
    _ = @import("BranchSnapshotsTests.zig");
