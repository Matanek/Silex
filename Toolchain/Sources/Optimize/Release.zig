...
pub const pass_descriptors = [_]PassDescriptor{
    ...
    .{ .id = .reference_memory_elision, .precondition = "dead pure reads removed and explicit same-block reference or scalar-view derivations available", .postcondition = "exact overwritten stores removed and exact post-store loads forwarded", .preserves = "possible alias observations, calls, block boundaries, ownership and diagnostics" },
