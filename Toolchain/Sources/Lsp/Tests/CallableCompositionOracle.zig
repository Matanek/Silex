...
const producer_cases = [_]ProducerCase{
    ...
    .{
        .id = "owned-bound-method",
        .producer = .bound_method,
        .canonical = "class Receiver { func candidate(value:int) bool { return true } func wrong(value:str) bool { return true } }\nclass Slot { let callback:func(int) bool }\nfunc make(receiver:Receiver) Slot { return Slot(callback:receiver.candidate) }\nfunc main() {}",
        .partial = "class Receiver { func candidate(value:int) bool { return true } func wrong(value:str) bool { return true } }\nclass Slot { let callback:func(int) bool }\nfunc make(receiver:Receiver) Slot { return Slot(callback:receiver.<|>) }\nfunc main() {}",
        .expected = .{ .method = .{ .owner = "Receiver", .name = "candidate" } },
        .forbidden = "wrong",
    },
    .{
        .id = "inherited-generic-listener",
        .producer = .bound_method,
        .canonical = "class Event<T> { var value:T }\nclass Node { func listen<T>(event:Event<T>, callback:func(T)) {} }\nclass Listener:Node { func candidate(value:int) {} func wrong(value:str) {} func ready(event:Event<int>) { self.listen(event, self.candidate) } }\nfunc main() {}",
        .partial = "class Event<T> { var value:T }\nclass Node { func listen<T>(event:Event<T>, callback:func(T)) {} }\nclass Listener:Node { func candidate(value:int) {} func wrong(value:str) {} func ready(event:Event<int>) { self.listen(event, self.<|>) } }\nfunc main() {}",
        .expected = .{ .method = .{ .owner = "Listener", .name = "candidate" } },
        .forbidden = "wrong",
    },
    .{
        .id = "inherited-event-property",
        .producer = .bound_method,
        .canonical = "class Listener:Area { func candidate(value:int) {} func wrong(value:str) {} func ready() { self.listen(self.entered, self.candidate) } }\nclass Area:Node { private var source:Event<int> = Event<int>(value:7); var entered:Event<int> { get { return self.source } } }\nclass Node { func listen<T>(event:Event<T>, callback:func(T)) {} }\nclass Event<T> { var value:T }\nfunc main() {}",
        .partial = "class Listener:Area { func candidate(value:int) {} func wrong(value:str) {} func ready() { self.listen(self.entered, self.<|>) } }\nclass Area:Node { private var source:Event<int> = Event<int>(value:7); var entered:Event<int> { get { return self.source } } }\nclass Node { func listen<T>(event:Event<T>, callback:func(T)) {} }\nclass Event<T> { var value:T }\nfunc main() {}",
        .expected = .{ .method = .{ .owner = "Listener", .name = "candidate" } },
        .forbidden = "wrong",
    },
    .{
