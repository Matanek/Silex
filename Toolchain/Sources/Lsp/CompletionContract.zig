...
pub const scenarios = [_]Scenario{
    ...
    .{
        .id = "declaration-operator",
        .capability = .declarations,
        .canonical_source = "struct Vec2 { let x:float }\nfunc operator +(left:Vec2, right:Vec2) Vec2 { return left }\nfunc main() {}",
        .partial_source = "struct Vec2 { let x:float }\nfunc op<|>",
        .required = &.{"operator"},
        .forbidden = &.{"struct"},
        .provenance = "FR/Language/Values/Operators",
        .status = .{ .protected = "Lsp.Completion: complete contextual operator declarations" },
    },
    .{
...
const function_field_decisions = [_]AstFieldDecision{
    ...
    .{ .name = "operator", .witness = "declaration-operator" },
