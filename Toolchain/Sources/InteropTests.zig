...
test "recognize macOS system math only from an isolated libSystem provider" {
    ...
    for ([_]Target{ .macos_arm64, .macos_x64 }) |target| {
        for ([_]bool{ true, false }) |standard| {
            ...
            const manifest = try std.fmt.allocPrint(allocator, "{{\"name\":\"MathProof\",\"version\":\"1.0.0\",\"boundary\":{{\"{s}\":{{\"providers\":{{\"System\":{{\"libraries\":{s}}}}}}}}}}}", .{ target.name(), libraries });
