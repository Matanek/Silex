...
pub fn validate(self: anytype) !void {
    for (self.program.structures, 0..) |declaration, structure_index| {
        ...
        if (declaration.base) |relation| if (protocolIndex(self, relation)) |protocol_index| {
            try validateOne(self, structure_index, declaration, protocol_index);
...
fn validateOne(self: anytype, structure_index: usize, declaration: Ast.Structure, protocol_index: usize) !void {
    ...
    for (protocol.methods) |requirement| {
        const implementation = findMethod(self, structure_index, requirement) orelse {
...
fn findMethod(self: anytype, structure_index: usize, requirement: Ast.Function) ?Ast.Function {
    var current: ?usize = structure_index;
    while (current) |candidate| : (current = self.structures[candidate].base) {
