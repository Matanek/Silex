...
pub const Session = struct {
    ...
    pub fn init(
        ...
        const cache_key = CompilationCache.artifactKey("semantic-package-graph-binary-v1", &.{ &graph_digest, target });
        const payload = CompilationCache.load(allocator, io, cache_key, "semantic-package-graph-binary-v1");
        var session: Session = .{
            .allocator = allocator,
            .io = io,
    ...
    pub fn loadSource(self: *Session, function: Ast.Function) ?Ir.Function {
        if (function.owner == 0 or function.owner >= self.packages.packages.len or function.is_anonymous or function.specialization_file != null) return null;
        ...
        const identity = self.sourceIdentity(function) orelse {
            self.function_misses += 1;
            return null;
        };
        for (artifact.functions) |cached| {
            if (cached.offset != function.position.offset or
                !std.mem.eql(u8, cached.path, identity.path) or
                !std.mem.eql(u8, cached.name, function.name)) continue;
    ...
    pub fn storeSources(self: *Session, source_functions: []const ?Ir.Function) void {
        if (self.loaded != null) return;
        const context = self.buildContext() catch return;
        var functions: std.ArrayList(CachedFunction) = .empty;
        for (self.program.functions, 0..) |source_function, function_id| {
            if (source_function.owner == 0 or source_function.owner >= self.packages.packages.len or
                source_function.is_anonymous or source_function.specialization_file != null) continue;
            if (function_id >= source_functions.len or source_functions[function_id] == null or
                !knownFunctionReferences(source_functions[function_id].?, self.program)) continue;
            const identity = self.sourceIdentity(source_function) orelse continue;
            functions.append(self.allocator, .{
                .path = identity.path,
                .offset = source_function.position.offset,
                .name = source_function.name,
                .function = source_functions[function_id].?,
            }) catch return;
        }
        ...
        const digest = CompilationCache.artifactKey("semantic-package-graph-binary-v1", &.{ &self.graph_digest, self.target });
        CompilationCache.store(self.allocator, self.io, digest, "semantic-package-graph-binary-v1", payload);
    }

    const SourceIdentity = struct { path: []const u8 };

    fn sourceIdentity(self: Session, function: Ast.Function) ?SourceIdentity {
        if (function.position.file >= self.files.len or function.owner >= self.packages.packages.len) return null;
        const root = self.packages.packages[function.owner].root;
        const relative = std.fs.path.relative(self.allocator, ".", null, root, self.files[function.position.file]) catch return null;
    ...
    fn buildContext(self: Session) !Context {
        ...
        const functions = try self.allocator.alloc(FunctionSymbol, self.program.functions.len);
...
fn knownFunctionReferences(function: Ir.Function, program: Ast.Program) bool {
    for (function.blocks) |block| for (block.instructions) |instruction| switch (instruction) {
        .function_reference => |value| if (!stableSourceFunction(program, value.function)) return false,
        .call => |value| if (!stableSourceFunction(program, value.function)) return false,
        .dynamic_call => |value| {
            if (!stableSourceFunction(program, value.function)) return false;
            for (value.implementations) |implementation| if (!stableSourceFunction(program, implementation.function)) return false;
        },
        .class_drop => |value| for (value.plans) |plan| for (plan.functions) |finalizer| {
            if (!stableSourceFunction(program, finalizer.function)) return false;
        },
        else => {},
    };
    return true;
...
fn stableSourceFunction(program: Ast.Program, index: usize) bool {
    if (index >= program.functions.len) return false;
