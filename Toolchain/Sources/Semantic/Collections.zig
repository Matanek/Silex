...
pub fn analyzeLiteral(
    ...
    const type_value = expected orelse return self.fail(position, "array literal requires an expected fixed array type");
    const structure_index = type_value.structureIndex() orelse return self.fail(position, "array literal requires an expected fixed array type");
    if (structure_index >= self.structures.len) return error.InvalidSource;
    const collection = self.structures[structure_index].collection orelse return self.fail(position, "array literal requires an expected fixed array type");
    const length = collection.length orelse return self.fail(position, "array literal requires an expected fixed array type");
    if (values.len != length) {
        const message = try std.fmt.allocPrint(self.allocator, "array literal expects {d} values, found {d}", .{ length, values.len });
        return self.fail(position, message);
    }
    const fields = try self.allocator.alloc(Ir.ValueId, values.len);
    for (values, 0..) |expression, index| {
...
pub fn analyzeCall(self: anytype, builder: anytype, call: Ast.Expression.Call) !?Model.TypedValue {
    ...
    _ = try self.analyzeExpression(builder, receiver_expression);
    const count = collection.length orelse return null;
    ...
    if (std.mem.eql(u8, call.name, "is_empty")) {
        ...
        try self.emit(builder, .{ .constant_bool = .{ .result = result, .value = count == 0 } });
