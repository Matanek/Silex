...
/// Wraps an already encoded Linux X64 image in a deterministic ELF64
...
pub fn emit(allocator: Allocator, code: []const u8, entry_offset: u32) Error![]u8 {
    ...
    try appendInt(allocator, &bytes, u16, 0x3e);
    ...
    try appendInt(allocator, &bytes, u32, 1);
    // X64 globals still live at the tail of the bootstrap image. Keep this
...
test "emit a deterministic Linux X64 ELF64 container" {
    ...
    const bytes = try emit(std.testing.allocator, &code, 0);
