...
bins={name:root/('view-loop-'+name) for name in ('before','after')}
def run(name):
...
data={'mode':'diagnostic','scope':'process wall time; scalar list allocation and startup retained; no LLVM comparator or physical X64 claim','warmup_pairs':6,'pairs':21,'host':platform.platform(),'machine':platform.machine(),'cpu':subprocess.check_output(['sysctl','-n','machdep.cpu.brand_string'],text=True).strip(),'binary_sha256':{k:hashlib.sha256(p.read_bytes()).hexdigest() for k,p in bins.items()},'source_sha256':hashlib.sha256((root/'KnownViewLoop.sx').read_bytes()).hexdigest(),'observations':rows,'summary':summary}
(root/'view-timing.json').write_text(json.dumps(data,indent=2)+'\n')
