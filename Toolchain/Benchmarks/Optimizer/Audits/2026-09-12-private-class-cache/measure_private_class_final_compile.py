...
for target in ['macos-arm64','macos-x64']:
 for i in range(13):
  for label in (['before','after'] if i%2==0 else ['after','before']):
   binary=p/('silex-after-direct-integer' if label=='before' else 'silex-after-private-class-target-policy')
   output=p/f'private-class-final-compile-budget-{target}-{label}'
   cmd=[str(binary),'compile',str(p/'NativeReference/Objects.sx'),'--release','--nocache','--target',target,'-o',str(output)]
   start=time.perf_counter_ns()
   with open(p/'private-class-final-compile-budget-last.log','w') as log:
...
(p/'private-class-final-compile-budget.json').write_text(json.dumps(dict(diagnostic_only=True,samples=11,warmups=2,summary=summary,observations=rows),indent=2)+'\n')
