...
p=Path(__file__).resolve().parent;names=('before','after','clang');binaries={n:p/('preparation-regional' if n=='after' else 'preparation-reference' if n=='before' else 'preparation-full-clang') for n in names};signature=json.loads((p/'preparation-full-correctness.json').read_text())['signature_interval']
def run(name):
...
for i in range(21):
 ...
 rows.append(row);(p/'preparation-regional-timing-partial.json').write_text(json.dumps(rows,indent=2)+'\n')
summary={}
...
(p/'preparation-regional-timing.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(summary,indent=2))
