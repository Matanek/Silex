...
for source,tag,expected in [('Silex/Toolchain/Benchmarks/Optimizer/Regressions/AggregatePreparation.sx','preparation',159),(str(p/'ValueModules/Main.sx'),'modules',8)]:
 for item in baseline['records']:
  ...
  (p/'dominance-targets.json').write_text(json.dumps(dict(compiler_sha256=hashlib.sha256(Path(cmd[0]).read_bytes()).hexdigest(),records=rows),indent=2)+'\n')
  assert row.get('passed'),row
print('24 emissions, 8 macOS executions agree')
