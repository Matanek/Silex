...
p=Path(__file__).parent;correct=json.loads((p/'snapshot-physics-correctness.json').read_text());results=[]
for stage in ['preparation','integration']:
 ...
 def run(name):
  ...
  value=stages.timing(r.stdout,stage,'clang-slots' if name=='clang' else 'silex',correct[stage]['signature_interval']);value['stdout']=r.stdout;return value
 for i in range(6):
