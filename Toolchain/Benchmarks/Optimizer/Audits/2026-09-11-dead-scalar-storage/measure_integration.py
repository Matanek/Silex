...
bins={name:root/('integration-'+('repeated' if name=='before' else 'shared')) for name in ('before','after')}
def run(name):
...
data={'mode':'diagnostic','scope':'process wall time for 1000000 two-body integration iterations; repeated versus explicitly shared mass-step; same compiler; no LLVM comparator or physical X64 claim','warmup_pairs':6,'pairs':21,'host':platform.platform(),'machine':platform.machine(),'cpu':subprocess.check_output(['sysctl','-n','machdep.cpu.brand_string'],text=True).strip(),'binary_sha256':{k:hashlib.sha256(p.read_bytes()).hexdigest() for k,p in bins.items()},'source_sha256':{name:hashlib.sha256((root/filename).read_bytes()).hexdigest() for name,filename in [('before','IntegrationRepeated.sx'),('after','IntegrationShared.sx')]},'observations':rows,'summary':summary}
(root/'integration-sharing-timing.json').write_text(json.dumps(data,indent=2)+'\n')
