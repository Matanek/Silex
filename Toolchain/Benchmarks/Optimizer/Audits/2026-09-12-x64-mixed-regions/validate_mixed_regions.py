...
records = []
for before in json.loads((p / 'gpr-exhaustive-binary-comparison.json').read_text()):
    cmd = before['command'].copy()
    cmd[0] = str(compiler)
    cmd[-1] = cmd[-1].replace('gpr-exhaustive-', 'mixed-regions-')
    run(cmd)
    binary = Path(cmd[-1])
    result = run([str(binary)])
    assert result.stdout == before['stdout']
    records.append(dict(command=cmd, workload=before['workload'], target=before['target'], sha256=sha(binary), identical_40dc120=sha(binary)==before['sha256'], stdout=result.stdout))
(p/'mixed-regions-binary-comparison.json').write_text(json.dumps(records, indent=2)+'\n')
print([(r['workload'], r['target'], r['identical_40dc120']) for r in records], flush=True)
...
for name, source, expected in fixtures:
    ...
    for target in targets:
        for mode in ['debug','release']:
            binary = p/f'mixed-regions-{name}-{target}-{mode}'
            ...
            records.append(row)
(p/'mixed-regions-native-validation.json').write_text(json.dumps(records, indent=2)+'\n')
shutil.copy2(compiler, p/'silex-after-mixed-regions')
(p/'mixed-regions-compiler.json').write_text(json.dumps({'sha256':sha(compiler)},indent=2)+'\n')
