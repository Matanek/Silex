...
pub fn run(
    ...
    const x64_stack = try Silex.Arm64Lower.lowerWithModeAndBoundaries(allocator, optimized, compilation.boundaries, .debug);
    const x64 = try Silex.X64RegisterAllocation.allocateProgram(allocator, x64_stack);
    const x64_function = findMachineFunction(x64, function_name) orelse return error.ContractFunctionMissing;
    try writeFile(io, output_directory ++ "/hot-budget-x64-machine.json", try std.json.Stringify.valueAlloc(allocator, x64_function, .{ .whitespace = .indent_2 }));
