...
compatibility, dependencies, namespace extension grants, and package contents
...
version to its repository, commit, archive checksum, manifest checksum, and
extension grants. Editing an installed manifest therefore cannot grant another
namespace. The local Git cache also refuses to replace an already observed tag
with another commit.
